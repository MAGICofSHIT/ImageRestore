% ----------------对一幅灰度图像进行运动模糊并叠加高斯噪声，并采用维纳滤波进行复原-------------
clc;         %清空控制台
clear;       %清空工作区
...
%-------------------------添加运动模糊---------`-------------
H_motion = fspecial('motion', 30, 180);%运动长度为18，逆时针运动角度为90°
...
figure('name','运动模糊加噪');
subplot(2,3,1);imshow(double_gray_pic,[]);title("原始灰度图");
subplot(2,3,2);imshow(motion_blur,[]);title('运动模糊');
subplot(2,3,3);imshow(motion_blur_noise,[]);title('运动模糊添加噪声');
...
subplot(2,3,4);imshow(restore_with_noise,[]);title('维纳滤波')

% 逆滤波
% inverseFilterImage = ifft(fourier_double_gray_pic + fourier_noise / fourier_H_motion);
% subplot(2,3,5);imshow(inverseFilterImage,[]);title('逆滤波')